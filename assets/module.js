(() => {
  ...
  const renderVisual = section => {
    ...
    if (visual) return `<figure class="concept-visual"><a class="concept-visual-link" href="${root}${visual.src}" target="_blank" rel="noopener" aria-label="Open ${esc(section.title)} diagram at full size"><img src="${root}${visual.src}" alt="${esc(visual.alt)}" loading="lazy" decoding="async"></a><figcaption>${esc(visual.caption)} <a href="${root}${visual.src}" target="_blank" rel="noopener">Open larger</a></figcaption></figure>`;
  ...
  const responseSupport = section => `<details class="support"><summary>I’m struggling — help me plan this response</summary><p><strong>What is this asking?</strong> Use the explanation in this section to make and support a clear point.</p><p><strong>Read → Select → Explain → Check</strong></p><ol><li>Read the question and return to <a href="#${esc(section.key)}">this theory section</a>.</li><li>Select the most relevant idea or example.</li><li>Explain how it answers the question.</li><li>Check that your answer is specific and uses your own words.</li></ol>${section.sentence_starters?.length ? `<p><strong>Sentence starter:</strong> ${esc(section.sentence_starters[0])}</p>` : ''}<p><strong>Appropriate response example:</strong> ${esc(section.appropriate_response_example)}</p></details>`;
